...
from .capsule_utils import _conv2d_wrapper
...
def baseline_model_cnn(X, num_classes):
    ...
    max_activation = tf.nn.max_pool(nets, ksize=[1, nets.shape[1], 1, 1], strides=[1, nets.shape[1], 1, 1],
                          padding='VALID', data_format='NHWC')
    print('****************max_activation.shape*********', max_activation.shape)
    nets = slim.flatten(max_activation)
...
def capsule_model_B(X, num_classes):
    ...
    for _, ngram in enumerate([3,4,5]):
        with tf.variable_scope('capsule_'+str(ngram)): 
            ...
            nets = capsules_init(nets, shape=[1, 1, 32, 8], strides=[1, 1, 1, 1],
                                 padding='VALID', pose_shape=8, add_bias=True, name='primary')
            nets = capsule_conv_layer(nets, shape=[3, 1, 8, 8], strides=[1, 1, 1, 1], iterations=3, name='conv2')
